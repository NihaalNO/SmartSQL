...
import {
  Database, History, BookmarkCheck, Search,
  Zap, LogOut, BarChart3, Shield,
} from "lucide-react"
import { logout, getUser } from "@/lib/auth"
import { clsx } from "clsx"

// ---------------------------------------------------------------------------
// Static config — defined outside component so it's never recreated
// ---------------------------------------------------------------------------
const ALL_NAV = [
...
export default function Sidebar() {
  // `mounted` starts false so that the initial client render matches the
  // server render (no sessionStorage on server → empty nav on server).
  // useEffect fires only after hydration is complete, at which point we can
  // safely read sessionStorage and re-render with the real user/role.
  ...
  // Before mount: null / "" — identical to what the server produces
  ...
  return (
    <aside className="w-60 min-h-screen bg-gray-900 text-gray-100 flex flex-col">
      ...
      {/* Nav — empty on server, populated after hydration */}
      ...
      {/* User info — hidden until mounted */}
